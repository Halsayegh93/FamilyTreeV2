-- ربط التسجيل بعضو الشجرة: العائلة المختارة عند التسجيل هي عائلته (طلب المالك)
-- merge_member_into_tree يُبقي السجل الجديد ويأخذ بيانات الشجرة؛ الآن تبقى عائلته
-- المختارة (أو عائلة سجل الشجرة إن لم يختر)، وتسري على أبنائه المنقولين وذرّيتهم.
...
begin
  -- ─── حماية: فقط فريق الإدارة يمكنه دمج/تفعيل الأعضاء ──────────
  -- بدون هذا كان أي مستخدم مسجّل يقدر يستدعي الدالة لحذف ملف أي عضو
  -- أو انتحال هويته (تُحدّث الدور/الحالة وتحذف السجل القديم).
  if not exists (
