...
// ---- Reads use the caller token so RLS also applies to AI context. ----
...
async function handleChat(payload: AIRequest, supabase: ReturnType<typeof getUserClient>) {
...
async function handleGenerateBio(payload: AIRequest, supabase: ReturnType<typeof getUserClient>) {
...
async function handleGenerateNews(payload: AIRequest, supabase: ReturnType<typeof getUserClient>) {
...
async function handleAdminSummary(payload: AIRequest, supabase: ReturnType<typeof getUserClient>) {
  ...
  if (!profile || !["owner", "admin", "monitor", "supervisor"].includes(profile.role)) {
...
async function handleAnalyzeTree(payload: AIRequest, supabase: ReturnType<typeof getUserClient>) {
...
serve(async (req) => {
  ...
  const auth = await authenticateRequest(req, ["member", "supervisor", "monitor", "admin", "owner"]);
  if (auth instanceof Response) return auth;
  const readClient = getUserClient(req.headers.get("authorization")!);
  ...
  // Never authorize or rate-limit by an identity supplied by the request body.
  payload.user_id = auth.profileId;
  const { action, user_id } = payload;
  if (action === "admin_summary" && !["owner", "admin", "monitor", "supervisor"].includes(auth.role ?? "")) {
    return json(403, { ok: false, message: "Admin access required" });
  }
  if (!action || !user_id) {
    return json(400, {
  ...
  try {
    switch (action) {
      case "chat":
        return await handleChat(payload, readClient);
      case "generate_bio":
        return await handleGenerateBio(payload, readClient);
      case "generate_news":
        return await handleGenerateNews(payload, readClient);
      case "admin_summary":
        return await handleAdminSummary(payload, readClient);
      case "analyze_tree":
        return await handleAnalyzeTree(payload, readClient);
      default:
